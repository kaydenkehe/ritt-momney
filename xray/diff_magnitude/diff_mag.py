# NOT DONE --- vanishing gradient when trying to incentivize large changes :(
...
for i, lrep in enumerate(lreps):
    ...
    for epoch in range(epochs):
        opt.zero_grad()
        latent_batch.append(lrep_par.clone().detach())
        ...
        loss = loss_fn(a, b, c, pred, lrep_par, latent_batch[-2] if epoch > 0 else lrep_par)
        ...
        print(f'Gradient Norm: {lrep_par.grad.norm().item()}')
