...
# differentiable histogram, given to me by Claude
def differentiable_histogram(x, num_bins=256, min_val=0.0, max_val=1.0):
    """
    ...
    centers = torch.linspace(min_val + bin_width/2, max_val - bin_width/2,
                             num_bins, device=x.device)
    centers = centers.view(1, -1)
    
    # Compute distances to bin centers
    dist = torch.abs(x - centers)
    
    # Compute weights using a triangular kernel
    weights = torch.max(torch.zeros_like(dist), 1.0 - dist / bin_width)
    
    # Normalize weights to sum to 1 for each input value
    weights = weights / weights.sum(dim=1, keepdim=True)
    
    # Sum weights across all input values to get the histogram
    hist = weights.sum(dim=0)
    
    return hist

# incentivize high outputs and low difference entropy
def loss_fn(a, pred, b, curr_img, prev_img):
    diff = (curr_img - prev_img.detach())[0]
    diff = diff.mean(dim=0)
    diff[0] += 1e-6 # prevent divide by 0
    diff = (diff - diff.min()) / (diff.max() - diff.min())

    hist = differentiable_histogram(diff)
    hist = hist / torch.sum(hist)
    hist += 1e-6
    ent = -torch.sum(hist * torch.log2(hist))
    print(hist.requires_grad, ent.requires_grad)

    print(f'Entropy: {ent.item():.2f}')

    return -a * pred + b * ent



a = -1 # incentivize low outputs
b = 0.05 # entropy coefficient
lr = 0.005 
epochs = 20
...
for i, lrep in enumerate(lreps):
    ...
    for epoch in range(epochs):
        ...
        loss = loss_fn(a, pred, b, img_orig,
